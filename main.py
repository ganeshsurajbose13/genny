...
# Better Analyze + Better Scanner + Index Scanner + ML + Breakout + Volume + Risk
# Render Optimized + Stable Version
...
SYMBOLS = sorted(list(set([
    ...
    "GAIL.NS","M&M.NS","DLF.NS","TATAPOWER.NS","VEDL.NS",
    ...
    "PAGEIND.NS","BHEL.NS","TATASTEEL.NS","ETERNAL.NS"
])))
...
def get_tf(symbol, interval="1d", period="6mo"):
    try:
        df = yf.download(
            symbol,
            interval=interval,
...
def analyze(symbol: str):
    ...
    volume = bool(latest["Volume"] > latest["VOL_AVG"])
    ...
    score += 10 if vwap_sig == "BULLISH" else -10
    score += 8 if volume else 0
    ...
    if "BUY" in signal or signal == "HOLD":
        risk = {
...
def scanner():
    ...
    for sym in SYMBOLS:
        try:
            ...
            score += 25 if latest["Close"] > latest["EMA20"] else -25
            score += 15 if latest["MACD"] > latest["MACD_SIGNAL"] else -15
            score += 12 if latest["Close"] > latest["VWAP"] else -12
            ...
            print(f"{sym} scanner error: {e}")
            continue

    return {
        "TOP_BUY": sorted(results, key=lambda x: -x["score"])[:20],
        "TOP_SELL": sorted(results, key=lambda x: x["score"])[:20],
        "TOTAL": len(results)
    }
...
@app.get("/index-scanner")
def index_scanner():
    results = []

    for name, symbol in INDEXES.items():
        try:
            df = get_tf(symbol, "1d", "6mo")
            ...
            results.append({
                "index": name,
                "price": round(f(latest["Close"]), 2),
                "rsi": round(f(latest["RSI"]), 2),
                "signal": signal,
